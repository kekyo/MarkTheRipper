...
`slug`とは、記事のカテゴリのようなものです。ここでは、`blog`と名付けていますが、以下のようにキーワード参照した場合:
...
例えば、記事カテゴリだけを管理するファイルとして、`resource/metadata-slug.json`のように別のファイルにしておけば、管理が容易になるでしょう。

この再帰検索は、一度のみ実行できます。つまり、得られた値をキーに繰り返し検索し続ける事は出来ません。

### イテレーターとネスト

tagやslugのような分類は、メニューから選択させてそのページに遷移させたいと思うでしょう。
例えば、サイト全体でtagが5個あったとします。これをページのメニューに自動的に加えて、
メニューからtagに分類されたページに遷移できるようにするには、イテレータ機能を使います。
...
「間にある文書」とは、ここでは ` '{tags-item}'` の事です。スペースが含まれてることに注意してください。
...
`tags`の`foo,bar`が、それぞれカンマで区切られて展開されて、クオートされて出力されました。
スペースをうまく巧妙に挿入しておくことで、見やすく区切られています。

繰り返しますが、`{foreach:tags}`と`{/}`の間にある文書が、繰り返し出力されるので、以下のように使う事も出来ます:
...
間に挿入されている`{tags-item}`は、繰り返される一つ一つの値を参照できるキーワードです。
`{<keyword>-item}` のような形式で指定します。`-item`は固定です。
同様に、`{<keyword>-index}` と指定すると、0から始まって1,2,3... とカウントする数値が得られます。

さらに、別のキーワードをネストさせる事も出来ます。例えば、slug一つ一つに対して、複数のtagを列挙する、ということも可能です。
slugやtagを使って例を示しましたが、当然、前節の再帰キーワード検索を適用することもできるため、
HTMLに出力するのは、人に見せたい名称にすることが出来ます。
