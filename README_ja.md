...
さて、この便利な関数を使用するためには、少しだけ準備が必要です。
この埋め込みコンテンツを表示させるための、専用のレイアウトファイル`embed.html`を用意し、`layouts`ディレクトリに配置します。
内容は以下の通りです:
...
そのような場合は、一緒に取得出来たoEmbedのメタデータを使用して、次に紹介する`card`関数と同様のコンテンツを生成します。
