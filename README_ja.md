...
tagやcategoryのような分類は、メニューから選択させてそのページに遷移させたいと思うでしょう。
...
さらに、別のキーワードをネストさせる事も出来ます。例えば、category一つ一つに対して、複数のtagを列挙する、ということも可能です。
categoryやtagを使って例を示しましたが、当然、前節の再帰キーワード検索を適用することもできるため、
